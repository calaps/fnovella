...
public class ParticipantSearch {
	...
	public Page<Participant> getResults(ParticipantRepository participantRepository, Pageable pageable) {
		...
		if (useName && useAppCode && useId) {
			List<Participant> participants = participantRepository.findByFirstNameStartingWithAndAppCodeAndId(this.firstName, this.appCode, this.id);
			...
			List<Participant> participants = participantRepository.findByFirstNameStartingWithAndId(this.firstName, this.id);
			return new PageImpl<>(participants, pageable, participants.size());
