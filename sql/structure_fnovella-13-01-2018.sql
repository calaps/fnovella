...
(
	...
	responsable int
		constraint program_activation_app_user_id_fk
			references app_user,
	...
	ns_nov int,
	ns_dec int,
	evaluation1 bit default 0,
	evaluation2 bit default 0
)
go
...
(
	...
	year_activation int,
	coordinator int
		constraint group_app_user_id_fk
			references app_user,
	ins_extra bit default 0,
	evs_extra bit default 0,
...
(
	...
	programActivation int
		constraint evaluation_program_activation_id_fk
			references program_activation,
	status int
)
go
...
create unique index participant_aditional_fields_values_id_uindex
	on participant_aditional_fields_values (id)
go
...
(
	id int identity
		primary key,
	group int
		constraint participant_aditional_fields_group_id_fk
			references group,
	participant int
		constraint participant_aditional_fields_participant_id_fk
			references participant,
	catalog int
		constraint participant_aditional_fields_catalog_id_fk
			references catalog,
	period int,
	year int
)
...
create unique index participant_aditional_fields_id_uindex
	on participant_aditional_fields (id)
go

alter table participant_aditional_fields_values
	add constraint participant_aditional_fields_values_participant_aditional_fields_id_fk
		foreign key (aditional_field_id) references participant_aditional_fields
go
